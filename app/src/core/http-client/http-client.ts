...
export class HttpClient {
  ...
  public get<T>(path: string, isAuthRequired: boolean = true): AsyncServiceResult<T> {
    return this._request<T>('GET', path, undefined, isAuthRequired)
  }

  public post<T>(path: string, data?: unknown, isAuthRequired: boolean = true): AsyncServiceResult<T> {
    return this._request<T>('POST', path, data, isAuthRequired)
  }

  public put<T>(path: string, data?: unknown, isAuthRequired: boolean = true): AsyncServiceResult<T> {
    return this._request<T>('PUT', path, data, isAuthRequired)
  }

  public patch<T>(path: string, data?: unknown, isAuthRequired: boolean = true): AsyncServiceResult<T> {
    return this._request<T>('PATCH', path, data, isAuthRequired)
  }

  public delete<T>(path: string, isAuthRequired: boolean = true): AsyncServiceResult<T> {
